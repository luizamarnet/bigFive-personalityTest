...
def polychoric_correlation(
...
) -> np.ndarray[tuple[int, int], np.dtype[np.float64]]:  # noqa: E501
    """
    ...
    try:
        ...
        with localconverter(ro.default_converter + pandas2ri.converter):
            ...
            bartlett_result = ro.conversion.get_conversion().rpy2py(
                ro.r("bartlett_result$p.value")
            )

        if kmo_result < 0.6:
            raise ValueError(
                "This dataset is not adequate for Factor Analysis. "
                "The overall KMO value for the dataset is bellow 0.6"
            )
        if bartlett_result[0] >= 0.05:
            raise ValueError(
                "P-value for Bartlett's test greater than 0.05."
                "Meaning the variables are uncorrelated and unsuitable for "
                "factor analysis."
            )
