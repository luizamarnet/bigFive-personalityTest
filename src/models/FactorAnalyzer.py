...
from src.visualization.visualization import plot_heatmap, plot_scree
...
def perform_factor_analysis(
    ...
    Parameters
    ----------
    correlation_matrix : np.ndarray (n_variables, n_variable)
        n_variable = 50 (the 50 personality items)
    ...
    fa = FactorAnalyzer(
        n_factors=len(column_names), rotation="varimax", is_corr_matrix=True
    )
    ...
    loadings: np.ndarray[tuple[int, int], np.dtype[np.float64]] = fa.loadings_
    ...
    for fator_idx in range(loadings.shape[1]):
        variaveis = [
            ...
            if fator_dominante[i] == fator_idx
        ]
