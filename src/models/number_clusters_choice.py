...
def number_clusters_choice(data: np.ndarray, max_k: int = K_RANGE) -> int:
    ...
    for k in k_range:
        ...
        if k >= 2:
            silhouette_scores.append(silhouette_score(data, clusters_k))

            davies_bouldin_scores.append(davies_bouldin_score(data, clusters_k))
    ...
    axes[1].plot(silhouette_k_range, silhouette_scores, marker="o")
    ...
    # Melhor silhouette
    best_silhouette_k = silhouette_k_range[np.argmax(silhouette_scores)]

    axes[1].axvline(best_silhouette_k, linestyle="--", alpha=0.7)

    axes[1].text(
        best_silhouette_k, max(silhouette_scores), f" Melhor k = {best_silhouette_k}"
    )
    ...
    axes[2].plot(silhouette_k_range, davies_bouldin_scores, marker="o")
    ...
    best_db_k = silhouette_k_range[np.argmin(davies_bouldin_scores)]

    axes[2].axvline(best_db_k, linestyle="--", alpha=0.7)

    axes[2].text(best_db_k, min(davies_bouldin_scores), f" Melhor k = {best_db_k}")
    ...
    logger.info(
        "Number of clusters analysis saved to: ./results/number_clusters_analysis.png"
    )
